...
module curved_textbox(txt, pos_h, font_size) {
  width = font_size * TEXT_SPACE_MULTIPLIER;
  half = width / 2;
  chars = len(txt);
  for (i = [0:chars - 1]) {
...
module measuring_line(txt, line_h, line_w, font_size, ratio) {
  pos_h = inner_h * ratio + BOTTOM;
  difference() {
    curved_line(line_h=line_h, line_w=line_w, pos_h=pos_h);
    curved_textbox(txt=txt, pos_h=pos_h, font_size=font_size);
  }
  curved_text(txt=txt, line_w=line_w, font_size=font_size, pos_h=pos_h);
}
...
union() {
  ...
  for (i = [0:len(MEASURINGS) - 1]) {
    ml = MEASURINGS[i];
    ratio = ml / TARGET_ML;
    measuring_line(txt=str(ml), line_h=LINE_HEIGHT, line_w=LINE_WIDTH, font_size=FONT_SIZE, ratio=ratio);
  }
}
